...
router.post("/verifyToken", verifyToken, async (req, res) => {
  ...
  const dataUser = await User.findOne({ email: req.user.user.email }).exec();

  if(dataUser){
    const user = { email: dataUser.email, role: dataUser.role };
    const token = generateToken(user, false);
    const refreshToken = generateToken(user, true);
    const data = {
      token: token,
    ...
    res.status(200).json({ Status: "Success verify token", data, error: null })
  }else{ 
